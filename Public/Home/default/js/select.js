$(function(){
	...
	$(".listValue").click(function(){
		var listValue_text = $(this).text();
                var listValue_value = $(this).attr('data-delid');              
		$(this).parent().parent().find(".customSelectValue").text(listValue_text);
                $('#cid').val(listValue_value);
		$(this).parent().hide();
	});
	/*控制显示个数，超出部分显示滚动条 */
	$(".customSelectStyle").each(function(){
	...
	function initStyle(obj){
		...
		obj.find(".customSelectBtn").css({
											...
											"right":"0",
											"width":"22px",
											"background":"url('"+ public_path +"/images/customSelectBtn.png')",
